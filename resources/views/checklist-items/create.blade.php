<x-industrial-layout>
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-3xl font-bold text-gray-900 flex items-center">
                    <i class="bi bi-plus-square mr-3 text-indigo-600"></i>
                    Add Checklist Item
                </h1>
                <p class="mt-1 text-sm text-gray-500">Create a checklist item with sub-items that will appear during maintenance checks.</p>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <form action="{{ route('checklist-items.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="nama_item" name="nama_item" value="{{ old('nama_item') }}" class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" placeholder="e.g. Cleaning" />
                        @error('nama_item')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4" class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" placeholder="Optional description">{{ old('deskripsi') }}</textarea>
                        @error('deskripsi')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <select id="tipe" name="tipe" class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent">
                            <option value="checkbox" {{ old('tipe') === 'checkbox' ? 'selected' : '' }}>Checkbox</option>
                            <option value="text" {{ old('tipe') === 'text' ? 'selected' : '' }}>Text</option>
                            <option value="number" {{ old('tipe') === 'number' ? 'selected' : '' }}>Number</option>
                    ...
                    <div>
                        ...
                        <input id="options" name="options" value="{{ old('options') ? (is_array(old('options')) ? implode(', ', old('options')) : old('options')) : '' }}" class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" placeholder="Option 1, Option 2, Option 3" />
                    ...
                    <div>
                        ...
                        <input type="number" id="urutan" name="urutan" value="{{ old('urutan', 0) }}" class="w-full rounded-xl border border-gray-300 px-4 py-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent" min="0" />
                        @error('urutan')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 gap-4">
                        <div class="flex items-center gap-3">
                            ...
                            <input type="checkbox" id="is_required" name="is_required" value="1" class="h-5 w-5 text-honda-red rounded focus:ring-honda-red" {{ old('is_required', true) ? 'checked' : '' }}>
                            <label for="is_required" class="text-sm text-gray-700">Required item</label>
                        </div>
                        <div class="flex items-center gap-3">
                            <input type="hidden" name="is_active" value="0">
                            <input type="checkbox" id="is_active" name="is_active" value="1" class="h-5 w-5 text-honda-red rounded focus:ring-honda-red" {{ old('is_active', true) ? 'checked' : '' }}>
            ...
            <div class="border-t pt-6">
                ...
                <p class="text-sm text-gray-600 mb-4">Add sub-items (e.g., "dalam", "1 frame", "2 roda") that will be checked during maintenance.</p>

                <div id="subItemsContainer" class="space-y-3">
                    @forelse(old('sub_items', []) as $index => $subItem)
                        @if(!empty($subItem['judul']))
                            <div class="subitem-row bg-gray-50 p-4 rounded-xl border border-gray-200 grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                                <div>
                                    ...
                                    <input type="text" name="sub_items[{{ $index }}][judul]" value="{{ $subItem['judul'] ?? '' }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="e.g., dalam" />
                                </div>
                                <div>
                                    <label class="block text-xs font-medium text-gray-700 mb-1">Description</label>
                                    <input type="text" name="sub_items[{{ $index }}][deskripsi]" value="{{ $subItem['deskripsi'] ?? '' }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="Optional" />
                                </div>
                                <div class="flex gap-2">
                                    <input type="number" name="sub_items[{{ $index }}][urutan]" value="{{ $subItem['urutan'] ?? $index }}" class="w-20 rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" min="0" />
    ...
    <script>
        ...
        let subItemCounter = {{ count(old('sub_items', [])) }};
        ...
        function addSubItem() {
            ...
            newRow.innerHTML = `
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Item Name *</label>
                    <input type="text" name="sub_items[${subItemCounter}][judul]" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="e.g., dalam" />
                </div>
                <div>
                    <label class="block text-xs font-medium text-gray-700 mb-1">Description</label>
                    <input type="text" name="sub_items[${subItemCounter}][deskripsi]" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" placeholder="Optional" />
                </div>
                <div class="flex gap-2">
                    <input type="number" name="sub_items[${subItemCounter}][urutan]" value="${subItemCounter}" class="w-20 rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-red-500" min="0" />
                    <button type="button" onclick="removeSubItem(this)" class="px-3 py-2 rounded-lg text-red-600 hover:bg-red-50 transition-colors">
                        <i class="bi bi-trash"></i>
                    </button>
                </div>
