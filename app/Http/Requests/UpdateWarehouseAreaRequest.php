...
class UpdateWarehouseAreaRequest extends FormRequest
{
    /**
    ...
    {
        $user = Auth::user();
        return $user && $user->can('areas.edit');
    }
    ...
    {
        $routeParam = $this->route('warehouse_area') ?? $this->route('warehouseArea');
        if (is_object($routeParam)) {
            $warehouseAreaId = $routeParam->id;
        } else {
            $warehouseAreaId = $routeParam ?? $this->input('id');
        }
        
        return [
            'name' => ['required', 'string', 'max:255', 'unique:warehouse_areas,name,' . $warehouseAreaId, 'min:2'],
